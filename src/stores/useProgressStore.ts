...
import type { CourseProgress, LessonProgress, UserId, CourseId, LessonId, ModuleId } from '@/types'
...
interface ProgressStore {
  ...
  markLessonComplete: (userId: UserId, courseId: CourseId, lessonId: LessonId) => void
  updateTimeSpent: (userId: UserId, lessonId: LessonId, timeSpent: number) => void
        ...
        markLessonComplete: (userId, courseId, lessonId) =>
          set((state) => {
            ...
            state.lessonProgress[lessonKey] = {
              ...
              lessonId,
              courseId,
              moduleId: existingProgress?.moduleId || ('module-1' as ModuleId), // Fallback
              status: 'completed',
            ...
            if (courseProgress) {
              const completedLessons = (Object.values(state.lessonProgress) as LessonProgress[]).filter(
                (progress) => 
              ...
              state.courseProgress[courseKey] = {
                ...courseProgress,
                completedLessons,
                progressPercentage: Math.round((completedLessons / courseProgress.totalLessons) * 100),
                lastAccessedAt: new Date(),
                status: completedLessons === courseProgress.totalLessons ? 'completed' : 'in-progress',
                completedAt: completedLessons === courseProgress.totalLessons ? new Date() : null,
              }
            }
          }),
...
export const useProgressError = () =>
  useProgressStore((state) => state.error)
